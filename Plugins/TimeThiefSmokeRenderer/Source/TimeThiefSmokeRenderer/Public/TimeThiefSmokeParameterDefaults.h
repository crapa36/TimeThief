...
namespace TimeThiefSmokeParameterDefaults
{
	...
	constexpr float ExplosionBoundsShiftScale = 0.0f;
	...
	//공기 상호작용 난류 강도.
	constexpr float TurbulenceStrength = 0.78f;
	//주변 공기 흐름이 속도장에 주는 강도.
	constexpr float AirInteractionStrength = 1.05f;
	//액터/폭발 이벤트 주변 국소 와류 강도.
	constexpr float EventVortexStrength = 1.15f;
	//와류 입자 개수.
	constexpr int32 VortexParticleCount = 64;
	//와류 입자 수명(초).
	constexpr float VortexParticleLifeSeconds = 1.45f;
	//와류 입자 속도장 주입 강도.
	constexpr float VortexParticleStrength = 140.0f;
	//와류 입자 영향 반경(cm).
	constexpr float VortexParticleSplatRadius = 125.0f;
	//와류 입자 중심 코어 반경(cm).
	constexpr float VortexParticleCoreRadius = 32.0f;
	//밀도 경계에서 와류 입자를 만들 민감도.
	constexpr float VortexDensityGradientScale = 3.8f;
	...
	constexpr float WarpTrailDecayRate = 11.5f;
	...
	//액터 warp 누적값 감쇠 시간(초).
	constexpr float ActorWarpAccumulationDecaySeconds = 0.38f;
	//액터 warp 생성 후 남기는 누적 비율.
	constexpr float ActorWarpEmissionRemainder = 0.22f;
	...
	//총알 통과 시 연막 제거 반경(cm).
	constexpr float BulletClearRadius = 15.0f;
	...
	//총알 wake 구멍 해제 페이드 시간(초).
	constexpr float BulletWakeReleaseDuration = 1.0f;
	//총알 wake 주변 재유입 억제 시간(초).
	constexpr float BulletWakeSinkLife = 1.0f;
	//총알 wake 주변 재유입 억제 강도.
	constexpr float BulletWakeSinkStrength = 0.5f;
	//총알 wake 진행 방향 속도장 충격 강도.
	...
	//렌더 밀도 노이즈 시간 스케일.
	constexpr float RenderNoiseTimeScale = 0.047f;
	//렌더 필라먼트 공간 스케일.
