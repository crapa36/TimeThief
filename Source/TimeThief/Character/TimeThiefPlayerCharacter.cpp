...
bool ATimeThiefPlayerCharacter::PurchaseItem(const FStoreOrder& Order)
{
    ...
    TMap<FGameplayTag, FUpgradeFloatLevels> DamageTable = UpgradeBalanceData ? UpgradeBalanceData->DamageBonusByWeaponAndLevel : DamageBonusByWeaponAndLevel;
    TMap<FGameplayTag, FUpgradeIntLevels> CapacityTable = UpgradeBalanceData ? UpgradeBalanceData->CapacityBonusByWeaponAndLevel : CapacityBonusByWeaponAndLevel;
    TMap<FGameplayTag, FUpgradeFloatLevels> RecoilTable = UpgradeBalanceData ? UpgradeBalanceData->RecoilReductionByWeaponAndLevel : RecoilReductionByWeaponAndLevel;
    TArray<float> MoveSpeedTable = UpgradeBalanceData ? UpgradeBalanceData->MoveSpeedBonusPerLevel : MoveSpeedBonusPerLevel;
    TArray<float> JumpVelocityTable = UpgradeBalanceData ? UpgradeBalanceData->JumpVelocityBonusPerLevel : JumpVelocityBonusPerLevel;

    const FTimeThiefGameplayTags& GameplayTags = FTimeThiefGameplayTags::Get();

    auto EnsureFloatLevels = [](TMap<FGameplayTag, FUpgradeFloatLevels>& Table, const FGameplayTag& Tag, std::initializer_list<float> Defaults)
    {
       if (!Tag.IsValid())
       {
          return;
    ...
    auto EnsureIntLevels = [](TMap<FGameplayTag, FUpgradeIntLevels>& Table, const FGameplayTag& Tag, std::initializer_list<int32> Defaults)
    {
       if (!Tag.IsValid())
       {
          return;
...
#if !UE_BUILD_SHIPPING
	      UE_LOG(LogTemp, Log, TEXT("[Store][SpeedUpgrade] FinalMoveSpeed(CombatBaseApplied) Base=%.2f Bonus=%.2f JumpFinal=%.2f"),
	         BaseMoveSpeed,
	         PS->AppliedUpgradeStats.MoveSpeedBonus,
